<?php
        ...
        if ($conn->connect_error) {
            ...
            $sql = "SELECT `Navegador`,count(`Navegador`) as parcial,(SELECT count(*) FROM DATOS) as total FROM DATOS GROUP BY `Navegador` ORDER BY parcial DESC";//CONSULTA
            $result = $conn->query($sql);
            
            if($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo '<p>'.$row["Navegador"].'</p>';
                    echo '<div class="w3-grey">';
                    $parcial= $row["parcial"];
                    $total= $row["total"];
                    $porcentaje= round(($parcial*100)/$total,2);
                    echo '<div class="w3-container w3-center w3-padding w3-green" style="width:'.$porcentaje.'%">'.$porcentaje.'%</div>';
                    echo '</div>';
                }
            }else{
